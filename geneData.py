...
def thetaParallel(para):
    equ_para['theta'], equ_para['kappa'] = para[0], para[1]
    exp1 = 'cos(kappa*(x[0]*cos(theta)+x[1]*sin(theta)))'
    exp2 = 'sin(kappa*(x[0]*cos(theta)+x[1]*sin(theta)))'
    uincR = fe.interpolate(fe.Expression(exp1, degree=3, kappa=equ_para['kappa'], \
                                         theta=equ_para['theta']), Vreal)
    ...
    return [measure.sampling(Fsol.uReal), measure.sampling(Fsol.uImag)]

p = Pool(5)
thetaLen = len(theta_all)
iter_num = 0
for kk in range(len(kappa_all)):
    print('frequncy is {:3}'.format(kappa_all[kk]))
    tempPara = np.array([theta_all, np.ones(len(theta_all))*kappa_all[kk]]).T
    temp = p.map(thetaParallel, tempPara)
    solR_all[:, iter_num:iter_num+thetaLen] = np.array(temp)[:,0,:].T
    solI_all[:, iter_num:iter_num+thetaLen] = np.array(temp)[:,1,:].T
    iter_num = iter_num + thetaLen
...
np.save('/home/jjx323/Projects/ISP/Data/dataRc', [solR_all, theta_all, \
        kappa_all, qFunStr])
np.save('/home/jjx323/Projects/ISP/Data/dataIc', [solI_all, theta_all, \
        kappa_all, qFunStr])
#np.save('/home/jjx323/Projects/ISP/Data/dataRsquare', [solR_all, theta_all, \
#        kappa_all, qFunStr])
#np.save('/home/jjx323/Projects/ISP/Data/dataIsquare', [solI_all, theta_all, \
#        kappa_all, qFunStr])
# -----------------------------------------------------------------------------
